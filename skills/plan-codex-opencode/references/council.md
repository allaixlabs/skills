...
규칙:
- **모든 패널 완료 알림 후에만** 결과 read (race 방지). 읽는 순서: manifest exit → result → log.
...
codex exec review --base "$BASE" -m gpt-5.5 -C "$RUN/wt/glm" \
...
**적용 후 정리**: 비채택 worktree는 cleanup이 제거. 비채택 브랜치는 `git -C "$ROOT" branch -D council/<...>`로 명시 삭제(채택/히스토리 보존 원하면 남김). `council_wt_cleanup` 최종 1회 + REPORT에서 `git worktree list` 잔존 0 확인.
