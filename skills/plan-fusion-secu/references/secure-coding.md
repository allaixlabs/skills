...
| 계층 | 항목 수 | 특징 |
|---|---|---|
| **순수 L1** | 17 | T1-1 인젝션·T1-3 시크릿·T1-7 CVE·T3-1 XSS·T3-3 Traversal·T3-5 ReDoS·T3-6 XXE·T3-7 Redirect·T3-10 하드코딩PW·T3-11 버퍼오버플로우·T3-12 NULL역참조·T3-13 정수오버플로우·T3-15 UAF·T3-17 응답분할·T3-18 약한난수·T3-16 클릭재킹·T4-2 Prototype Pollution |
| **L1+L2 혼합** | 20 | T1-2 암호화·T1-4 접근제어·T1-6 설정오류·T1-8 인증로직·T1-9 역직렬화·T1-10 SSRF·T2-2 LLM민감정보·T2-5 LLM출력처리·T2-6 과도한권한·T2-7 시스템프롬프트누출·T2-10 DoS·T3-2 CSRF·T3-4 역직렬화상세·T3-8 파일업로드·T3-9 레이스컨디션·T3-19 정보누출·T3-20 TLS·T4-1 예외오처리·T4-3 Mass Assignment |
| **순수 L2** | 3 | T1-5 설계·T3-14 로깅누락·T2-9 환각대응 |
| **L2+L3** | 2 | T2-4 데이터독성·T2-8 벡터DB — 설계 판단은 L2, 외부 provenance는 L3 |
| **L1+L2+L3** | 1 | T2-3 LLM 공급망(lockfile=L1, 패키지검증=L2, 모델평판=L3) |
| **L3 단독** | 0 | (L3는 항상 L1/L2와 짝) |
| **총합** | **43** | 17+20+3+2+1 (재검토 2026-06-23 직접 집계로 정정 — 이전 14+18+5+3+1=41은 산술 오류) |

**L2 담당 합계**: 순수 L2 3 + L1+L2 혼합의 L2 부분 20 + L2+L3의 L2 부분 2 + L1+L2+L3의 L2 부분 1 = **26개**.
...
⚠️ **🔒 L1 raw 시크릿 노출 주의 (재검토 2026-06-23 확인)**: `$RUN/l1-raw/` 의 gitleaks/trufflehog raw 결과(gitleaks.json·trufflehog.jsonl)에는 **실제 시크릿 값이 평문**으로 포함될 수 있다.
- REPORT에 `$RUN/l1-raw/` 경로를 공개할 때, 이 디렉토리가 외부에 노출(로그 공유·PR 첨부·채팅 붙여넣기)되면 시크릿이 유출된다.
- **규칙**: raw 파일의 *내용*은 후보/최종 답변/Judge 프롬프트에 **절대 붙이지 마라** — 경로만 언급. 실제 시크릿 값은 gitleaks 보고서의 요약(RuleID·파일·라인)만 쓰고 value는 마스킹.
- 산출물 공유 전 `l1-raw/` 하위 파일의 secret 값 grep 후 마스킹(`sed`) 권장.
