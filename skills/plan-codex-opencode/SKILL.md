...
3. **모드 선택**:
   ...
   - 파일 변경 없는 분석/리서치/설계 질문 → **Council-Research**(read-only 사본 격리 기본; codex `-s read-only`, opencode·omo는 사본+`--dangerously-skip-permissions`)
...
4. **격리 폴더**:
   ...
   RUN=$(umask 077; mktemp -d "${TMPDIR:-/tmp}/pco.${slug}.XXXXXX") || { echo "RUN 생성 실패" >&2; exit 1; }
...
## 3. DELEGATE (모드별 위임 — 백그라운드 + 패널별 manifest, 능동 폴링)

> 공통: 각 패널은 **별도 Bash `run_in_background: true`**(한 셸에서 `&` 금지). 산출물 `$RUN/<id>/`. 다중 패널은 병렬이 본질이라 백그라운드가 기본이되, **수동 대기(턴 종료 후 알림만 기다림)는 금지** — 다음 응답마다 **능동적으로** 각 패널의 exit 파일(`$RUN/<id>/exit.txt` 또는 manifest)을 폴링해 완료 수를 센다. 전원 완료 확인 시 즉시 결과 read → §4 VERIFY/SYNTHESIZE로 넘어간다 — "기다리겠다"며 멈추거나 진행 없이 안내문만 내놓지 않는다(부분 완료 진행 보고는 허용).
> **race 가드**: 패널 결과 파일(`result.md`/`round1.log`)은 해당 패널 완료 *후에만* 읽는다(완료 전 read는 빈/직전 결과). 단, exit/manifest로 진행 상태 카운트하는 것은 완료 전에 허용.
...
```
정리는 REPORT 직전 `council_wt_cleanup "<root>" "$RUN"`로 명시 호출한다. setup·위임·정리를 한 Bash 호출에 묶는 경우가 아니면 `trap ... EXIT`를 쓰지 않는다.
...
codex `-s read-only -C "<root>"`(샌드박스 강제). opencode/omo는 원본 루트가 아니라 참가자별 읽기전용 사본(`$RUN/ro/<id>`, `.git` 제외·심링크 차단)에서 `--dangerously-skip-permissions`로 실행한다. 사후 `git status`는 원본 오염 보조 탐지일 뿐이며, 격리 실패 시 동일 루트로 폴백하지 않는다(`references/council.md` §1).

### Pipeline — 구현 → 리뷰 → 수정 → 종합
> 격리: 구현도 baseline 보존을 위해 worktree에서 한다 — `council_wt_setup "<root>" "$RUN" "$slug" impl` 후 `$RUN/wt/impl`에서 작업하고, 종합 후 `council_wt_adopt "<root>" "$RUN" impl`로 메인 반영(단일 패널이라 worktree 1개). REPORT 직전 `council_wt_cleanup "<root>" "$RUN"`로 정리한다. 단일트리 직접 구현이면 `baseline.status`로 사후 범위 검증.
1. **구현**: 모델 A를 `omo run --agent Sisyphus -d "$RUN/wt/impl"`(완수보장), SESSION_A 추출.
2. **리뷰** (리뷰어 패밀리 ≠ 구현자 패밀리): 구현자가 omo(glm/kimi)면 `( cd "$RUN/wt/impl" && codex exec review --uncommitted )`. **구현자가 codex면** opencode엔 `exec review`가 없으니 diff를 브리프에 담아 glm/kimi에 일반 위임한다(`references/council.md` §3-(b) 패턴).
3. **수정**: 리뷰 지적을 모델 A에 resume으로 되돌림(omo `--session-id` / opencode `-s` / codex `exec resume`). Claude가 직접 안 고침.
4. → 4단계 종합으로.

위임 산출물 세트(패널별): `$RUN/<id>/manifest`, `$RUN/<id>/round1.log`; codex 패널만 `-o`로 `$RUN/<id>/result.md`를 만든다. omo/opencode 패널은 `round1.log`가 result 역할이다. session_id 추출은 백엔드별(`references/`).
...
2. **코드**: worktree별 diff 추출(`baseline.head` 대비) → 접근 차이 분석 → **교차리뷰**(다른 패밀리가 리뷰, `codex exec review --base`) → 판정(단일 채택 / 장점 합성).
   교차리뷰 redirect 전 `mkdir -p "$RUN/xreview"`를 먼저 실행한다.
...
- **UI 노출 작업이면(HANDOFF 'UI 노출 판정=yes')**: 디자인 스펙(타이포/컬러/간격/레이아웃) 반영 여부 + UI Acceptance Criteria(예: 주요 화면 before/after 스크린샷 대조) 충족을 명시 대조. 구현이 스펙을 안 따르거나 UI가 빠졌으면 **FAIL → 백엔드 resume/재위임**(Claude 자가치유 금지 — 역할 경계).
- 미달 시 최종 적용 백엔드 **세션 resume**(최대 3라운드, `ORCHESTRATION_FAIL`은 미산입). 방향이 틀렸으면 fresh 재위임.

### loop-md 연동
루트에 `loop.md` 있으면 loop-md 연동은 다음 순서로 고정한다: `council_wt_adopt` → **메인 ROOT에서** loop-md Verify 전체 ①Pass/Fail·②정량·③정성 실행 → `.loop/last-verified`가 현재 HEAD인지 확인 → 그 다음 커밋. **패널 worktree에서의 검증은 사전검증일 뿐 hard-guard 마커 갱신이 아니다** — worktree는 메인 작업트리의 `.loop/last-verified`를 갱신하지 못하므로(loop-md SKILL.md 참조), adopt 후 반드시 메인에서 Verify를 다시 실행해야 마커가 현재 HEAD로 갱신된다. **③정성의 독립 검증자는 council 교차리뷰로 자연 충족** — synthesis.md의 교차리뷰를 ③ 근거로 연결. 루트에 `loop.md` 없으면 이 절차(마커 갱신) N/A.
...
- REPORT 직전 `council_wt_cleanup "<root>" "$RUN"` 1회 호출 + **`rm -rf "$RUN/ro"`(Council-Research 참가자 읽기전용 사본 — `council_wt_cleanup`은 `wt/`·`council/*`만 다루고 `ro/`는 안 지운다 → 디스크·민감코드 사본 누수)**
- **누수 점검**: `git -C "<root>" worktree list`에 council worktree 잔존 0 + `git -C "<root>" branch --list 'council/*'`에 council 브랜치 잔존 0 + `[ ! -d "$RUN/ro" ]` 확인(1줄)
