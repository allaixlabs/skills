...
- **codex(GPT)**: `codex exec --skip-git-repo-check -C <dir> -m gpt-5.5 -c model_reasoning_effort="xhigh" -o result.md - < handoff.md`. 비코드는 `-s read-only`. ⚠️ **`--skip-git-repo-check` 필수** — `-C` 대상이 git repo 미인식(심링크 경로·서브디렉토리·non-repo)이면 `Not inside a trusted directory`로 즉시 exit 1(read-only여도). **교차리뷰 1급 도구 `codex exec review --base <BASE>`**. 상세 `codex-cli.md`.
