...
> **모델명 SSOT**: 아래 코드 예시의 모델 문자열(`gpt-5.6-sol`·`zai-coding-plan/glm-5.2`·`"Gemini 3.1 Pro (High)"`·
> `opencode-go/kimi-k2.7-code`·`opencode-go/deepseek-v4-pro`)은 `models.yaml`(단일 진실원)에서 온다.
> 버전업·모델명 변경 시 **models.yaml 만 고치고 `sync-models.sh`** — `check-models.sh` 가 이 파일과
> SSOT 정합을 자동 검증(드리프트 시 FAIL). `check-fusion.sh` 는 `models.lib.sh` 를 source 해 `$M_*_CLI`
> 변수로 읽지만, 아래는 사람이 읽는 예시이므로 값을 그대로 표기한다.
...
>
> ⚠️ **GLM 예외(참가자 한정)**: `ORCH_FAMILY=glm`이면 opencode(GLM)는 동족이어도 **참가자에 필수 포함**(`GLM_MANDATORY_PARTICIPANT=yes`) — 역할 분리(오케스트레이터=검증 only·불가양도 / opencode 참가자=독립 풀이)로 동족 위험 완화 + '최소 3종 백엔드(codex·agy·glm)' 보장. 동종할인(`PARTICIPANT_CONFLICT_RISK=partial`)을 synthesis/REPORT에 명시. **Judge·Synth는 여전히 동족 회피**(참가자만 예외).
...
codex exec --skip-git-repo-check -C "$RUN/wt/codex" -m gpt-5.6-sol -c model_reasoning_effort="xhigh" \
...
echo "family=opencode" >> "$RUN/glm/manifest"   # glm은 opencode 백엔드 → quorum에선 opencode 1 family로 집계(단 kimi opencode-go는 별도 provider라 동족 판정 분리)
echo "model=zai-coding-plan/glm-5.2" >> "$RUN/glm/manifest"   # -m 형식 사후검증용(exit≠0 시 'Model not found' 원인 대조)

# kimi (opencode 직접)
# ⚠️ -m 는 항상 'provider/model' 전체 — 베어 모델명이나 끝 슬래시(kimi-k2.7-code/)면
#    opencode가 'Model not found'로 exit=1 (위임 자체 실패). routing-fusion.md 정규화표의
#    model열 문자열을 그대로 복사. 위임 직후 사후검증으로 슬래스 형식을 grep 확인한다(아래 _check_model_args).
...
echo "model=opencode-go/kimi-k2.7-code" >> "$RUN/kimi/manifest"   # -m 형식 사후검증용(exit≠0 시 'Model not found' 원인 대조)

# ⚠️ -m provider/model 형식 사후검증(위임 직후 1회): 각 opencode/omo 참가자의 manifest에
#    model 문자열을 기록해 두면, 폴링 단계에서 'Model not found' exit=1 의 원인이 -m 형식
#    오타(끝 슬래시·prefix 누락)였는지를 기계적으로 가린다. 형식이 틀리면 즉시 재위임 전 정정.
#    (위 위임 블록에 echo "model=opencode-go/kimi-k2.7-code" >> "$RUN/kimi/manifest" 를 함께 기록하면 좋다.)
```

> **`-m` 형식 가드(opencode/omo 전용)**: 위 kimi/glm/deepseek 위임에선 `-m`을 **`provider/model` 단일 슬래시**로 넣어야 한다. 끝에 슬래시가 붙거나(`opencode-go/kimi-k2.7-code/`) provider prefix가 빠지면(`kimi-k2.7-code`) opencode가 `Model not found`로 exit=1 → 해당 참가자는 `round1_exit=1`로 무응답 처리돼 quorum에 영향. 각 참가자 위임 후 manifest에 `model=<사용한 -m 값>`을 한 줄 기록해 두면, exit≠0일 때 §3-1 전 ·폴링 단계에서 로그(`Model not found`)와 대조해 -m 오타를 바로 찾을 수 있다.

규칙:
- **완료 알림을 수동으로 기다리지 말 것 (능동 폴링 우선)** — 하네스가 "You will be notified when it completes"를 반환해도 그것을 **유일한 트리거로 삼아 턴을 종료하면 안 된다**(실제로 알림이 수 시간 누락된 사례가 있었다 — 그 동안 에이전트가 멈춰 사용자가 재촉해야 했다). 각 참가자 위임(`run_in_background: true`) 직후 **같은 응답 내에서 즉시 능동 폴링 루프**를 이어간다: 짧은 `sleep`(예: 15~60초) 후 `$RUN/<id>/exit.txt`·manifest exit·`round1.log` 크기 변화를 점검하는 것을 반복하며, 매 응답마다 완료 수를 센다(예: "2/3 완료, 나머지 실행 중"). 전원 완료(exit 확보) 시 즉시 종합 절차로 넘어간다 — "기다리겠다"며 멈추거나 진행 없이 안내문만 내놓지 않는다(부분 완료 진행 보고는 허용).
- **완료 전 결과 read는 race**(완료 후에만 의미 있는 결과). 능동 폴링은 **진행 상태 카운트**(exit/manifest/로그 꼬리)만 확인하고, 결과 파일 read는 해당 참가자 exit=0/1 확보 **후**에. manifest exit → codex `result.md`, 그 외 `round1.log`.
- **이 규칙은 글로벌 가드(`async-polling-guard`)가 강제한다** — loop-md Setup §6이 모든 에이전트(zcode/claude/codex) 글로벌 지침에 배포. 스킬 문서·레퍼런스뿐 아니라 글로벌 가드가 최후의 방어선이라, 컨텍스트 요약/세션 교체 후에도 살아있다.
...
- 동일 패밀리 중복 금지(gpt-5.6-sol + gpt-5.6-sol-fast 같은 조합은 독립성 0).
...
while [ -n "$_rest" ]; do
  ...
  _model="${_tail%:*}"                    # opus / gpt-5.6-sol / opencode-go/deepseek-v4-pro / glm ...
...
>
> **런타임 폴백 체인(F3)**: claude(기본 Judge)가 런타임에 죽어도(주간 한도·인증 만료·E2BIG 등) 즉시 `self`로 직행하지 않는다. check-fusion.sh가 `JUDGE_FALLBACK_CHAIN`(claude→codex→agy→opencode-deepseek→opencode-glm/kimi→self, 총 6후보)을 산출하고, 위 루프가 차순위 후보로 자동 전환한다. **GLM 예외 + DeepSeek 예외**: `ORCH_FAMILY=glm`이면 opencode(GLM)는 **GLM 예외로 참가자에 필수 포함**(`GLM_MANDATORY_PARTICIPANT=yes`, 동종할인 `PARTICIPANT_CONFLICT_RISK=partial`)되며, DeepSeek 라우트(`opencode-go/deepseek-v4-pro`)와 glm/kimi 라우트는 Judge 폴백 체인의 별도 후보로도 살아남는다(동종할인 경고 `judge_conflict=partial` — synthesis.md에 명시). 이것이 "claude가 죽어 Judge를 잃는다"를 막는 핵심 경로다.
...
- **동족 경고(일반화)**: Judge 백엔드가 오케스트레이터 패밀리와 같으면(`JUDGE_CONFLICT_RISK=yes` 또는 `judge_conflict=partial`/`yes`) `synthesis.md`에 명시:
  ...
  - `partial` = Judge가 오케스트레이터와 백엔드 공유(opencode-deepseek, GLM 오케스트레이터) → "Judge 부분 비독립(동종할인 — opencode 백엔드 공유)". **GLM 참가자 동종도 `partial`**: `ORCH_FAMILY=glm`이면 opencode(GLM) 참가자가 오케스트레이터와 동족(`PARTICIPANT_CONFLICT_RISK=partial`) → "참가자 1패밀리 동종(GLM) — 역할 분리(오케스트레이터=검증 only / 참가자=독립 풀이)로 동종할인" 표기.
  - `no` = 비독립(할인 불필요)
...
| race(완료 전 read) | 완료 알림 **전** read 금지(race); 알림 도착 즉시 read → 종합(대기/안내문 금지) |
