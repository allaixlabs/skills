...
4. **격리 폴더**:
   ...
   fence="CANDIDATE_DATA_$(od -An -N6 -tx1 /dev/urandom 2>/dev/null | tr -d ' \n' || echo "$$$RANDOM")"
   printf 'mode=%s\npanel=%s\njudge=%s\nsynth=%s\nfence=%s\n' "<모드>" "<참가자 id들>" "<judge>" "<synth>" "$fence" >> "$RUN/manifest"
   ```
...
**omo 폴백**: `check-fusion.sh`가 `OMO_RUN_READY=no`(또는 `OMO_PLUGIN_REGISTERED=no`)면 GLM도 **opencode 직접 경로**(`opencode run --variant high`)로 위임한다 — 기본 패널 예시는 omo를 가정하나 omo 미준비 시 자동 폴백.
