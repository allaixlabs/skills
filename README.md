...
| **lottie-director** | 러프한 요청 → 공식 **Prompt guide 5원칙** 브리프 보강 → text-to-lottie 위임 | "로티 만들어줘", "가이드대로 로티" | text-to-lottie 스킬 |
...
### 🎬 lottie-director — Prompt guide 5원칙 브리프 보강
러프한 "로티 만들어줘"를 [diffusionstudio/lottie](https://github.com/diffusionstudio/lottie) 공식 **Prompt guide 5원칙**(① 자산 그라운딩 ② 모션 디자인 용어 ③ 카메라 연출 ④ 컨트롤 명시 ⑤ FPS·길이)을 채운 감독급 브리프로 보강한 뒤, 실제 제작은 설치된 `text-to-lottie` 스킬에 위임한다. 모호어→모션 용어 번역표("통통 튀게"→spring overshoot)와 타입별 기본값(로더/로고/차트/프로모별 fps·frames·카메라·컨트롤)으로 빈 칸만 채우며, 사용자가 명시한 값은 덮어쓰지 않는다. 보강한 항목은 위임 후 투명하게 공개. → [상세](skills/lottie-director/README.md)
...
| **aside** (선택) | 브라우저 증거 수집(DOM·스크린샷·PDF) — `mcp__aside__repl` MCP 도구 또는 `aside repl` CLI | [Aside 브라우저 앱 설치](https://aside.ai) → `aside` CLI 포함, `aside --update`로 갱신 | plan-fusion(브라우저 증거 필요 시) — [`skills/plan-fusion/references/aside-repl.md`](skills/plan-fusion/references/aside-repl.md) 참조 |
...
└── lottie-director/     # 러프 요청 → 공식 Prompt guide 5원칙 브리프 보강 → text-to-lottie 위임
```
