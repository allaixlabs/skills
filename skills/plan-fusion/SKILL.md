...
codex `-s read-only`(강제, live 루트 안전). **codex 외 전 백엔드(agy·opencode·omo·claude)는 강제 샌드박스가 없으므로 읽기전용 `cp -a` 사본에서 `--dangerously-skip-permissions`로 실행**한다 — 쓰기가 떨어져도 사본이라 원본 무해(예방), 사후 `git status`는 보조 탐지. 백엔드별 들쭉날쭉을 없앤 통일 규칙(`references/fusion.md` §1). 과거 "권한 프롬프트가 차단(skip 미사용)" 가정은 헤드리스 미검증이라 폐기.
