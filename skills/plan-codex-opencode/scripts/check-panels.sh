...
if [ "$codex_ok" = 1 ] && [ "$opencode_ok" = 1 ]; then
  ...
  if [ "$opencode_ok" = 1 ]; then
    echo "HINT: opencode 백엔드만 가용 — 그 안의 서로 다른 패밀리(예: glm+kimi)로 council 독립성 확보 가능. codex(GPT) 추가 시 패밀리 다양성↑." >&2
  else
    echo "HINT: codex(GPT) 백엔드만 가용 — 단일 모델 패밀리뿐이라 교차검증 독립성이 성립하지 않습니다. council 대신 plan-then-codex(단일 위임)를 쓰거나, opencode를 설정해 다른 패밀리를 확보하세요." >&2
  fi
  exit 0
else
