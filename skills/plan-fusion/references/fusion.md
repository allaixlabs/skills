...
규칙:
- **모든 참가자 완료 알림 후에만** 결과 read (race 방지). **모든 참가자의 완료 알림이 도착하면 즉시** 결과 read → 종합 절차로 넘어간다 — "기다리겠다"며 멈추거나 진행 없이 안내문만 내놓지 않는다(부분 완료 진행 보고는 허용)("전 read 금지"는 알림 **후** 진행이 아니라 **전** race만 막는다). manifest exit → codex `result.md`, 그 외 `round1.log`.
