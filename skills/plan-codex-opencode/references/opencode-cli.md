...
| 우선순위 | 권장 경로 | 이유 |
|---|---|---|
| 완수보장 우선 | **omo run** (Sisyphus) | 긴 구현·다단계·병렬 서브태스크를 끝까지 완수 |
| 고추론(variant) 우선 | **opencode run 직접** | `--variant high`로 provider-specific reasoning 지정 (omo엔 없음) |
| 둘 다 필요 | **opencode run 직접 + 상위 모델 변형** | 완수보장은 포기하고 고추론·변형 조합으로 단발 위임 |
| 리뷰 / 분석 / 2nd opinion | **opencode run 직접** | 단발·경량·N개 병렬 용이 |

요약: 구현의 끝까지 완수가 중요하면 `omo run`을, `--variant`를 포함한 고추론이 중요하면 `opencode run` 직접을 쓴다. 두 가치를 동시에 요구하면 `opencode run` 직접에 상위 모델 변형을 조합한다.
...
| 모델 인증 오류 | `opencode providers list`로 해당 provider `●` 확인 (check-panels.sh의 매트릭스) |
