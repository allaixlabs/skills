...
Codex는 `loop.md`가 없으면 `~/.claude/skills/loop-md/scripts/detect-stack.sh`로 **직접 부트스트랩**도 가능 — Setup·Verify를 Claude 없이 단독 수행한다.

## 옵트인: hard 가드 (커밋 차단)

soft 가드(글로벌 CLAUDE.md 지시)로 부족하면, **커밋 시점에 강제**하는 PreToolUse hook을 켠다.
`~/.claude/settings.json` 의 `hooks.PreToolUse` 배열에 추가:
```json
{ "matcher": "Bash", "hooks": [
  { "type": "command", "command": "bash \"/Users/<you>/.claude/skills/loop-md/scripts/precommit-guard.sh\"", "timeout": 5 } ] }
```
- `loop.md` 있는 프로젝트에서 `git commit` 시, Verify 마커(`.loop/last-verified`)보다 나중에 수정된 **추적 소스가 있으면 차단**(`.pyc` 등 빌드 산출물은 mtime 판정에서 제외).
- Verify 통과 시 `touch .loop/last-verified` 로 마커가 갱신되어 커밋이 허용된다.
- **우회**(자가치유·긴급): 커밋 메시지에 `[skip-loop]` 또는 `LOOP_SKIP=1`.
