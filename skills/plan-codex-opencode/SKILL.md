...
4. **격리 폴더**:
   ...
   ```
5. **패널·모드·각 백엔드 플래그를 사용자에게 1회 요약**하고 진행. 인간 승인 영역(스키마/보안/키·시크릿/결제/배포/PRD범위/아키텍처)이면 여기서 중단.
...
1. **구현**: 모델 A를 `omo run --agent Sisyphus`(완수보장), SESSION_A 추출.
2. **리뷰**: `codex exec review --uncommitted`로 **타 패밀리**가 미커밋 변경 리뷰(구현자가 codex였으면 리뷰어는 glm/kimi).
3. **수정**: 리뷰 지적을 모델 A에 `--session-id` resume으로 되돌림(Claude가 직접 안 고침).
4. → 4단계 종합으로.
...
- **누수 점검**: `git -C "<root>" worktree list`에 council worktree 잔존 0 확인(1줄)
