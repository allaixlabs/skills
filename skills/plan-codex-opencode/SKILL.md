...
2. **사용자에게 종합 결과·변경 파일·AC 충족을 먼저 보고** (다른 동기 스킬처럼 완료 즉시 알림).
3. **그 다음** **메인 ROOT에서** loop-md Verify 전체 ①Pass/Fail·②정량·③정성 실행 → `.loop/last-verified`가 현재 HEAD인지 확인 → 커밋.
**패널 worktree에서의 검증은 사전검증일 뿐 hard-guard 마커 갱신이 아니다** — worktree는 메인 작업트리의 `.loop/last-verified`를 갱신하지 못하므로(loop-md SKILL.md 참조), adopt 후 반드시 메인에서 Verify를 다시 실행해야 마커가 현재 HEAD로 갱신된다. **③정성의 독립 검증자는 council 교차리뷰로 자연 충족** — synthesis.md의 교차리뷰를 ③ 근거로 연결. 루트에 `loop.md` 없으면 이 절차(마커 갱신) N/A.
