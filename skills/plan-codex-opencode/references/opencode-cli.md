...
검증 기준: omo(oh-my-openagent) 4.9.2 · opencode 1.16.2 — `omo run --help` / `opencode run --help` 실측.
...
⚠️ JSON 필드명(`session_id` / `sessionId` / `id`)은 omo/opencode 버전에 따라 다를 수 있다. 추출 실패가 곧 실패는 아니며, resume이 필요할 때만 확정하면 된다.

## Resume (VERIFY 라운드)

```bash
# omo
$OMO_BIN run "검증 지시…" --session-id "$SESSION_ID" -d "<원 디렉토리>" --json > "$RUN/<id>/roundN.log" 2>&1
# opencode
...
| session_id 추출 실패 | round1.log에서 `session` 키 직접 확인 — resume 필요할 때만 |
| resume 즉사 | session_id 오입력/만료 → 새 세션 재위임 |
