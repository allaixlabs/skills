...
현재 레포의 변경을 리뷰한다. 이 스킬에서 **다른 패밀리(GLM/Kimi)의 변경을 GPT가 독립 리뷰**하는 데 쓴다.
...
```bash
codex exec review --base "$BASE" -m gpt-5.5 \
  -C "$RUN/wt/glm" \
...
```bash
codex exec -C "$ROOT" -s read-only -o "$RUN/codex/answer.md" - < "$RUN/handoff-research.md"
```
...
3. 읽는 순서: manifest exit → `result.md` → 이상하면 `round1.log` 에러.
