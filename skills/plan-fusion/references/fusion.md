...
규칙:
- **모든 참가자 완료 알림 후에만** 결과 read (race 방지). **완료 알림(`Background task completed` / `task-notification`)이 도착하면 즉시** 결과 read → 종합 절차로 넘어간다 — "기다리겠다"며 멈추거나 안내문만 출력하지 않는다("전 read 금지"는 알림 **후** 진행이 아니라 **전** race만 막는다). manifest exit → codex `result.md`, 그 외 `round1.log`.
