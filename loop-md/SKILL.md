...
6. **완료 검증 가드 (글로벌 우선 — 프로젝트마다 주입 금지)**: 프로젝트 `CLAUDE.md`에 주입하지 않는다(매번 복붙·중복·누락 유발). 대신 **글로벌 가드가 있는지 확인**한다:
   ```bash
   grep -q "loop.md" ~/.claude/CLAUDE.md && echo "글로벌 가드 있음" || echo "없음 → 1회 설정 제안"
   ```
   - 없으면 글로벌 `~/.claude/CLAUDE.md`에 **조건부 1줄**을 1회 추가 제안: *"프로젝트 루트에 `loop.md`가 있으면 완료 선언 전 반드시 loop-md Verify로 ①②③ 리포트(실행 증거 포함)를 출력. 없으면 무시(N/A)."* (Codex는 `~/.codex/AGENTS.md`에 대칭 추가.)
   - 이미 있으면 그대로 둔다. **조건부라 `loop.md` 없는 프로젝트엔 무해**하므로 1회 설정으로 전 프로젝트가 커버된다.
   - hard 강제를 원하면 PreToolUse(`git commit`) 가드 hook을 **옵트인**으로 안내(Stop hook은 매 턴 발동해 노이즈가 크므로 권하지 않음).
