...
# 형식
omo run [옵션] "<message>"
bunx oh-my-openagent run [옵션] "<message>"   # omo 별칭 없을 때
```
...
`--json` 출력에서 sessionId(omo 4.10.0 실측)를 추출:

```bash
SESSION_ID=$(grep -o '"sessionId":"[^"]*"' "$RUN/round1.log" | head -1 \
  | sed 's/.*"sessionId":"\([^"]*\)".*/\1/')
...
⚠️ omo 4.10.0 `--json` 세션 필드명은 `sessionId`다.
...
JSON 내 session ID 필드명은 opencode 명령의 별도 출력으로 확인해야 한다.
...
| sessionId 추출 실패 | round1.log에서 `session` 키 직접 확인 |
| resume 즉사 | sessionId 오입력 또는 세션 만료 — 새 세션으로 재위임 |
