...
|---|---|---|---|---|
| **default**(호명 없을 때) | codex `gpt-5.6-sol` · agy `"Gemini 3.1 Pro (High)"` · opencode `glm-5.2` · opencode `kimi-k2.7-code` | claude(Opus) | codex(GPT) | 일반 — 4개 모델(백엔드 3: codex·agy·opencode)로 다양성 확보 |
| **highEnd** | gpt-5.6-sol · **Opus 4.8(claude)** · Gemini 3.1 Pro · glm-5.2 | Opus ⚠️비독립 | GPT | 고성능(Opus 참가) |
...
|---|---|---|---|---|---|---|
| codex | `-C <dir>` | `-m gpt-5.6-sol` | `-c model_reasoning_effort="<v>"` | `exec resume <id>` | `-o FILE` | `--sandbox workspace-write` |
