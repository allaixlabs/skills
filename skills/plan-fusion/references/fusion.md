...
rsync -a --safe-links --exclude '.git' --exclude node_modules "$ROOT/" "$RO/" 2>/dev/null \
  || { rm -rf "$RO"; cp -a "$ROOT" "$RO" && rm -rf "$RO/.git" && find "$RO" -type l -delete; }
  # ↑ rsync `--safe-links`=트리 밖 심링크만 제외(트리 안 심링크 보존). cp 폴백은 `find -type l -delete`로
  #   ⚠️ cp 전에 `rm -rf "$RO"` — rsync가 $RO를 만든 뒤 중간 실패하면 $RO가 이미 존재해 `cp -a "$ROOT" "$RO"`가
  #     $RO/<basename ROOT>/로 '중첩'되고(에이전트가 파일 못 찾음) `rm -rf "$RO/.git"`도 빗나가 .git이 누출된다.
  #   모든 심링크 제거(BSD/GNU find 이식성 위해 보수적) → 폴백 사본이 약간 더 강하게 격리될 수 있음(무해).

# ⚠️ Fusion-Research는 council_wt_setup을 호출하지 않으므로 참가자 출력 디렉토리가 없다 → 위임 전 직접 생성한다.
#    (없으면 아래 `> "$RUN/<id>/round1.log"`·codex `-o "$RUN/codex/result.md"` 리다이렉트가
#     'No such file or directory'로 깨진다. Fusion-Code는 council_wt_setup이 `$RUN/<id>`를 만들어 무관.)
mkdir -p "$RUN/codex" "$RUN/$id"
...
- **부분 실패 + quorum**: 죽은 참가자는 "무응답". 단 **생존 참가자가 서로 다른 패밀리 ≥2**라야 Fusion 종합을 진행한다(교차검증 독립성의 최소 조건). 생존 패밀리가 1개뿐이면 그건 더 이상 교차검증이 아니므로 **단일 위임 결과 + "Fusion 미성립(1패밀리 생존)" 표기**로 격하한다. 이 규칙은 §3-1의 `family=` 카운트로 **기계적으로** 확인한다(문서 규칙에만 의존하지 않음). CLI/인증/플래그 오류 = `ORCHESTRATION_FAIL`(라운드 미산입, 그 참가자만 재시도).
- agy/omo는 백그라운드 wall-clock 상한을 둔다(예: 30분). agy는 `--print-timeout`이 자체 차단하지만 Claude 쪽 상한도 병행.
- **handoff도 argv 주의**: agy/opencode/omo는 handoff를 `"$(cat handoff.md)"` argv로 받는다 — handoff가 비정상적으로 크면(대형 baseline diff 등) 참가자 호출도 `E2BIG` 위험. handoff는 스펙만 담아 적정 크기로 유지(codex는 `- < FILE` stdin이라 무관). **사전 가드**(위임 직전 1회): `sz=$(wc -c < "$RUN/handoff.md"); [ "${sz:-0}" -gt 131072 ] && echo "WARN: handoff ${sz}B(>128KB) — argv E2BIG 위험. baseline diff 등을 빼 스펙만 남기거나 codex(stdin) 참가자 위주로." >&2`.
...
# ⚠️ manifest에 fence=가 없으면(비정상) 정적 'CANDIDATE_DATA'로 굳지 말고 즉석 난수 재생성 — 정적 토큰은 후보가
#    예측·하드코딩해 데이터펜스를 이스케이프할 수 있다. 폴백도 hex/숫자뿐이라 아래 `sed s/${FENCE}/.../`의 구분자 안전.
[ -n "$FENCE" ] || FENCE="CANDIDATE_DATA_$(od -An -N6 -tx1 /dev/urandom 2>/dev/null | tr -d ' \n')"
FENCE=${FENCE:-CANDIDATE_DATA_$$$RANDOM}
{
...
if [ "${fam_n:-0}" -lt 2 ]; then
  echo "WARN: 생존 백엔드 패밀리 ${fam_n}종(<2) — Fusion 미성립. FUSE를 중단하고 단일 위임 결과 + 'Fusion 미성립' 표기로 격하하라(§2 quorum)." >&2
fi
```
...
rsync -a --safe-links --exclude '.git' --exclude node_modules "$ROOT/" "$RO_REV/" 2>/dev/null \
  || { rm -rf "$RO_REV"; cp -a "$ROOT" "$RO_REV" && rm -rf "$RO_REV/.git" && find "$RO_REV" -type l -delete; }   # rm 선행: rsync 부분실패 시 cp 중첩 방지(§1과 동일)
