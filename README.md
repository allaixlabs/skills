...
| **aside** (선택) | 브라우저 증거 수집(DOM·스크린샷·PDF) — `mcp__aside__repl` MCP 도구 또는 `aside repl` CLI | `npm i -g aside` (또는 플랫폼 `aside-browser` 스킬) | plan-fusion(브라우저 증거 필요 시) — [`skills/plan-fusion/references/aside-repl.md`](skills/plan-fusion/references/aside-repl.md) 참조 |
