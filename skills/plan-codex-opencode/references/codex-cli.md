...
Council/Pipeline에서 codex 패널은 자기 작업 디렉토리(`-C`)에서 실행한다 — Council-Code면 격리 worktree(`-C "$RUN/wt/codex"`), 비코드/단일트리면 프로젝트 루트(`-C "$ROOT"`).
...
| 옵션 | 용도 |
|---|---|
| `--uncommitted` | 스테이징·언스테이징·추적되지 않은 변경 전부 리뷰 (Pipeline 구현 직후) |
| `--base <BRANCH>` | 주어진 base 브랜치 대비 변경 리뷰 (Council: 패널 worktree를 baseline 대비) |
| `--commit <SHA>` | 특정 커밋이 도입한 변경 리뷰 |
| `-m <MODEL>` | 리뷰어 모델 |
| `--title <TITLE>` | 리뷰 요약에 표시할 커밋 제목 |
| `[PROMPT]` / `-` | 커스텀 리뷰 지시 (stdin은 `-`) |

예 — Council에서 glm 패널 worktree의 변경을 GPT가 리뷰:
```bash
mkdir -p "$RUN/xreview"
...
```
`read-only`면 모델이 파일을 쓰려 해도 샌드박스가 차단 → 비코드 Council에서 안전.
...
- 코드 작업 기본: `--sandbox workspace-write` — 쓰기를 `-C` 루트(+`--add-dir`)로 한정. Council worktree와 결합하면 패널 격리가 이중으로 보장된다.
...
| `git stash create`/`git diff` 결과가 비정상 (council_wt_adopt가 "bad revision"/"No valid patches"로 ABORT) | PATH의 `git`이 wrapper(`rtk git` 등)여서 출력을 가로챈 것. `type git`으로 wrapper 여부 확인 → wrapper면 `/usr/bin/git`으로 council-worktrees.sh가 실제 git을 쓰도록 PATH 조정(`PATH=/usr/bin:$PATH` 스크립트 상단에 명시는 부작용 위험 → 차선: wrapper를 비활성화한 셸에서 스킬 실행). **스크립트는 stash create 출력을 SHA(40hex)로 검증해 노이즈를 빈 값으로 정규화**한다(비-SHA면 변경 없음 취급). |
